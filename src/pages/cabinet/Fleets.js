...
import {useAuth,dbPackages,dbImages} from '../auth/FirebaseConfig';
import {MinLoder,formatDate, Toast, chackeVal,idGenerator,useFileName} from "../Utils";
import {ImageCropper} from "../modals/ImageTools";

export const Fleets = ({language,mode}) =>{
  return(
...
export const FleetModal = ({language}) =>{
  ...
  const [states, setStates] = useState({loading: false,onSaved:false});
  const [datas,setDatas]=useState({id:idGenerator(),image:"", name:"",maturity:30, percentage:5,min:0,max:0, depositedIn:0,status:true});
  const [images, setImages] = useState({id:datas.id,src:""});
  const [error,setError]=useState({image:null,name:null,maturity:null, percentage:null, min:null,max:null, error:{text:null,stack:null}});
  const [fileName, setFileName,clearFileName] = useFileName(null);
  const datasOuter = {
    "set":{title:texts.newFleet[language]},
    "update":{title:texts.updateTitle[language]+" "+texts.fleet[language]}
  }
  
  useEffect(()=>{
    if(id && isMounted.current){
      dbPackages.child(id).on("value", (snapChat)=>{
        if(snapChat.exists()){
          let imageItem = snapChat.val();
          dbImages.child(imageItem.id).on("value",(snapImage)=>{
            if(snapImage.exists()){
              setDatas(imageItem);
              setImages(snapImage.val());
            }else{setDatas(imageItem);}
          });
        }
      });
    }
  ...
  const handleChange =(event)=>{
    const fieldsNum=  ["maturity","percentage","min","max"]
    const {name,value} = event.target;
    let number;
    for(let j in fieldsNum){
      if(name === fieldsNum[j]){number = parseInt(value);}
    }
    setDatas(prevData=>({...prevData,[name]:number?number:value}));
  } // em cada input em digitação atualiza o valor no state
  
  const handleSubmit = (e) => {
    e.preventDefault(); 
    const errors = new Array();
    const v = ["maturity","percentage","min","max"];
    if(datas.name.length < 2){
      setError(prevError=>({...prevError,name:`${texts.invalid[language]} ${texts.name[language]}`}));
      errors.push(1);
    }
    for(let k in v){
      if(datas[v[k]] <= 0 || !datas[v[k]]){
        setError(prevError=>({...prevError,[v[k]]:texts[v[k]][language]+" "+texts.cannotBe0OrNull[language]}));
        errors.push(1);
      }
    }
    if(errors.length <= 0){
        ...
        <div className="modal_body">
          <div className="input_card flex_c_c">
            <div className="modal_avatar flex_c_c">
              ...
              {images.src && <img src={images.src} className="avatar" alt="fleet"/>
          ...
          <div className="input_card">
            <div className="input_wrap flex_b_c">
              <input onChange={handleChange} onFocus={handleClearError} value={datas.maturity} className={chackeVal(datas.maturity, "input")} id="maturity" name="maturity" type="number" readOnly={states.loading}/>
               <label htmlFor="maturity">{texts.maturity[language]}</label>
               </div>
            <div className="label_error"> {error && error.maturity}</div>
          </div>
           <div className="input_card">
            <div className="input_wrap flex_b_c">
              <input onChange={handleChange} onFocus={handleClearError} value={datas.percentage} className={chackeVal(datas.percentage, "input")} id="percentage" name="percentage" type="number" readOnly={states.loading}/>
               <label htmlFor="percentage">{texts.percentage[language]}</label>
               </div>
            <div className="label_error"> {error && error.percentage}</div>
          </div>
          <div className="input_card">
            <div className="input_wrap flex_b_c">
              <input onChange={handleChange} onFocus={handleClearError} value={datas.min} className={chackeVal(datas.min, "input")} id="min" name="min" type="number" readOnly={states.loading}/>
               <label htmlFor="min">{texts.min[language]}</label>
               </div>
            <div className="label_error"> {error && error.min}</div>
          </div>
          <div className="input_card">
            <div className="input_wrap flex_b_c">
              <input onChange={handleChange} onFocus={handleClearError} value={datas.max} className={chackeVal(datas.max, "input")} id="max" name="max" type="number" readOnly={states.loading}/>
               <label htmlFor="max">{texts.max[language]}</label>
               </div>
            <div className="label_error"> {error && error.max}</div>
          </div>
